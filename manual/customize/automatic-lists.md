...
Now we need to enable automatic list creation in Sympa. To do so, we have to:
  ...
  - set the [`automatic_list_feature`](../man/sympa.conf.5.md#automatic_list_feature) parameter to `on` and define who can create automatic lists via the [`automatic_list_creation`](../man/sympa.conf.5.md#automatic_list_creation) (points to an `automatic_list_creation` scenario);
...
You can make Sympa delete automatic lists that were created with zero list members; to do so, you should set the [`automatic_list_removal`](../man/sympa.conf.5.md#automatic_list_removal) parameter to `if_empty`.
